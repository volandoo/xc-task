...
const recalcDistance = (waypoints: LatLng[]) => {
    ...
    if (waypoints.length > 1) {
        for (let i = 0; i < waypoints.length - 1; i++) {
            const distance = computeDistanceBetweenLatLng(waypoints[i], waypoints[i + 1]);
...
const getShortestPath = (points: Point[], esIndex: number, goalLine: boolean, zone: number) => {
    const tolerance = 1.0;
    ...
    while (!finished && opsCount-- > 0) {
        const { distance, line } = optimizePath(points, count, esIndex, goalLine, zone);
        // See if the difference between the last distance id
        // smaller than the tolerance
        finished = lastDistance - distance < tolerance;
        lastDistance = distance;
        if (line) goalline = line;
    }
    return { distance: lastDistance, goalline };
};
...
// esIndex - index of the ESS point, or -1
// line - goal line endpoints, or empty array
const optimizePath = (points: Point[], count: number, esIndex: number, goalLine: boolean, zone: number) => {
    let distance = 0;
    let line: LatLng[] = [];
...
// esIndex - index of the ESS point, or -1
const getTargetPoints = (points: Point[], count: number, index: number, esIndex: number) => {
...
const getIntersectionPoints = (c: Point, a: Point, b: Point, distAB: number) => {
...
const processLine = (c: Point, a: Point, zone: number) => {
    const goalLine: ShortPoint[] = [];
    const prevCoords = utm2degress(c.x, c.y, zone);
    const goalCoords = utm2degress(a.x, a.y, zone);

    const goal = { lat: goalCoords[1], lon: goalCoords[0] };
    const prev = { lat: prevCoords[1], lon: prevCoords[0] };
    let lastLegHeading = computeHeading(goal, prev);
    ...
    const firstPoint = computeOffset(prev, c.radius, heading);

    // Reversing the heading.
    heading += 180;
    // And now completing the line with a point 100m further.
    const secondPoint = computeOffset(firstPoint, 2 * c.radius, heading);
...
const degrees2utm = (lon: number, lat: number, zone: number) => {
    if (!utms[zone]) {
        utms[zone] = proj4("+proj=utm +zone=" + zone, "+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs");
    }
...
const computeOffset = (latLng1: LatLng, radius: number, heading: number): LatLng => {
    let gl = new GeodesicLine.GeodesicLine(geod, latLng1.lat, latLng1.lon, heading);
...
const createCylinders = (waypoints: Waypoint[], goalLine: LatLng[]): turf.Feature[] => {
    return waypoints.map((waypoint, index) => {
        if (goalLine.length && index === waypoints.length - 1) {
            return {
                type: "Feature",
                properties: {
                    type: "line",
                ...
                geometry: {
                    type: "LineString",
                    coordinates: goalLine.map((t) => [t.lon, t.lat]),
                },
            };
        }
        const color =
        ...
        return {
            ...
            geometry: createCircle(waypoint.latLng.lon, waypoint.latLng.lat, waypoint.radius),
        };
    });
};
...
const processTask = (turnpoints: Waypoint[], goalType?: "cylinder" | "line", geojson?: boolean) => {
    ...
    const points: Point[] = [];
    for (let i = 0; i < turnpoints.length; i++) {
        const p = degrees2utm(turnpoints[i].latLng.lon, turnpoints[i].latLng.lat, zone);
        points.push(createPoint(p[0], p[1], turnpoints[i].radius));
    }

    const { distance, goalline } = getShortestPath(points, es, goalType === "line", zone);
